...
/**
 * Class: ZG_Table
 * Description: Allows for quick creation/deletion of WP Tables
 * 
...
class ZG_Table {
	...
	function install() {
		zg_install_table( $this->name, $this->columns, $this->upgrade_method );
	}

	function uninstall() {
		zg_uninstall_table( $this->name );
	}
}
 ...
 */
function zg_register_table( $key, $name = false, $global= false ) {
	global $wpdb;
	...
	if( $global ) {
		$wpdb->global_tables[] = $name;
		$wpdb->$key = $wpdb->base_prefix . $name;
	} else {
		$wpdb->tables[] = $name;
...
function zg_install_table( $key, $columns, $upgrade_method = 'dbDelta' ) {
	global $wpdb;
...
function zg_uninstall_table( $key ) {
	global $wpdb;
